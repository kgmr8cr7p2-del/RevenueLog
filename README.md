...
- Telegram-уведомления доверенным пользователям: когда прошла половина срока сборки и за 2 дня до дедлайна.
...
Если архив не сохраняется и карточки возвращаются на основную доску, значит сайт уже обновлен, а Web App в Apps Script все еще работает на старой версии. После вставки нового `Code.gs` обязательно сделайте `Deploy -> Manage deployments -> Edit -> New version -> Deploy`.
...
При запуске на ПК батник также откроет сайт в браузере на весь экран. Сначала используется Chrome или Edge, если они установлены.
