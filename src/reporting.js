...
const REPORT_COLUMNS = [
  ...
  ['contractFileUrl', 'Файл договора'],
  ['paymentDate', 'Дата оплаты'],
  ['shippingDate', 'Дата отправки'],
  ['receivedDate', 'Дата получения'],
  ['assemblyStartDate', 'Дата отсчета срока'],
  ['assemblyTermDays', 'Срок на все, дней'],
  ['buildDeadline', 'Дедлайн'],
  ['paidRub', 'Оплата, руб'],
...
function getReportRows(builds) {
  return builds.map((build) => ({
    ...
    trackingNumber: build.trackingNumber || '',
    contractFileUrl: build.contractFile?.url || '',
    paymentDate: build.paymentDate || '',
    shippingDate: build.shippingDate || '',
    receivedDate: build.receivedDate || '',
    assemblyStartDate: build.assemblyStartDate || '',
    assemblyTermDays: build.assemblyTermDays || '',
